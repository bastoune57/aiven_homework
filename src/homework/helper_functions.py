...
def generate_json_message(userId=-1):
    """! Function that generates a json string
    with user id, datestamp and random coordinates information

    @param userId indicates the userID. If not given,
    or a negative value, a random number
    between 0 and 9 will be chosen.
    @return json_str A json formatted string with
    the userId, datestamp and random coordinates information
    """
    ...
    # get random latitute (in range [-90;+90]°)
    # and longitude (in range [-180;+180]°) for positionning
...
def validate_record_format(record):
    """! Function that validates the format of the records
    read from the kafka server and to be pushed to the postgresql db
...
def validate_coordinates (record):
    ...
    if latitude < -90 or latitude > 90:
        raise Exception("Coordinate latitude is out of range: "
    ...
    if longitude < -180 or longitude > 180:
        raise Exception("Coordinate longitude is out of range: "
                "{} (should be between -180 and 180)".format(longitude))
